...
def display_head(f):
    df_load = pd.read_csv(f, sep=",", nrows=10)
    print(df_load.head())
display_head(fichier2)
display_head(fichier)
## On réfléchit en terme de lignes
## créer un dictionnaire par soucis de vitesse


def completeness_per_country(df):
    missing_values_per_country = defaultdict(lambda: {"missing": 0, "N_case": 0})

    for i in range(len(df)):
        row = df.iloc[i]
        cc = row["country_code"]  
        missing_case = row.isnull().sum() 
        N_case_per_country = row.count()

        missing_values_per_country[cc]["missing"] += missing_case
        missing_values_per_country[cc]["N_case"] += missing_case + N_case_per_country

    # Dictionnaire global contenant les pourcentages de complétude
    completeness_results = {}

    print("Complétude par pays:")
    for country, case in missing_values_per_country.items():
        missing = case["missing"]
        N_case_tot = case["N_case"]
        completeness_percentage = ((N_case_tot - missing) / N_case_tot) * 100  

        completeness_results[country] = {
            "missing": missing,
            "N_case": N_case_tot,
            "completeness_percentage": completeness_percentage
        }

        print(f"Pays : {country}, Pourcentage de complétude : {completeness_percentage:.2f}%")
...
def uniqueness_per_country(df):
    unique_countries_code = df["country_code"].unique()
    uniqueness_result = {}
    for cc in unique_countries_code:
        row_per_country = df[df["country_code"] == cc]
        N_doublon = row_per_country.duplicated().sum()
        uniqueness_percent = (N_doublon / len(row_per_country)) * 100
        print(f"le nombre de doublon pour le pays {cc} vaut {uniqueness_percent}%")
        uniqueness_result[cc] = {
...
def result():
    ...
    results_df.to_excel("resultats_par_pays.xlsx", index=False)
